...
Usage:
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    GLint location_indexring = glGetUniformLocation(prog_id, "index_ring");
    GLint location_countring = glGetUniformLocation(prog_id, "count_ring");

    glUniform1i(glGetUniformLocation(prog_id, "count_ring"), totalring);
    ...
    glUniform1i(glGetUniformLocation(prog_id, "is_moon"), false);
    glUniform1i(glGetUniformLocation(prog_id, "index_ring"), false);
    glBindVertexArray(VAO);
    glDrawElements(GL_PATCHES, F.size(), GL_UNSIGNED_INT, 0);
    

    glUniform1i(glGetUniformLocation(prog_id, "is_moon"), true);
    for(int i = 0;i<totalring;i++)
    {
      glUniform1i(location_indexring, i);
